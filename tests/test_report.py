...
from src.report import build_report, export_pdf, uc5_section
...
class TestBuildReport:
    def test_creates_markdown_with_all_sections(self, tmp_path):
        uc2 = _write_csv(
            tmp_path,
            "uc2.csv",
            [{"country": "FR", "target": "gptinc_p0p100_992_j", "variable": "mgdpro", "correlation": 0.95}],
        )
        uc3 = _write_csv(
            tmp_path,
            "uc3.csv",
        ...
        uc4 = _write_csv(
            tmp_path,
            "uc4.csv",
            [{"target": "gptinc_p0p100_992_j", "variable": "shweal", "r_historical_abs": 0.75, "r_cross_abs": 0.77}],
        )
        path = build_report(
        ...
        for section in ["## 2. UC-2", "## 3. UC-3", "## 4. UC-4", "## 5. UC-5", "## 6. Финальный"]:
